...
export const handleBeeperStatusChange = async ( beeper: Beeper,req: Request,res: Response): Promise<void> => {
  ...
  switch (beeper.status) {
    ...
    case BeeperStatus.shipped:
      ...
      await new Promise((resolve) => setTimeout(resolve, 10000));//  מחכה 10 שניות לשינוי הסטטוס ומעדכן
      beeper.status = BeeperStatus.detonated; (beeper.name = ` ${beeper.name} kiled`),  (beeper.detonated_at = new Date()),
